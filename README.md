...
- **Merge to `main`** (or a manual **workflow_dispatch**) builds `dist/` in CI
  and deploys it to GitHub Pages via `upload-pages-artifact` + `deploy-pages`.

`dist/` is **gitignored and never committed**. GitHub Actions builds it on every
deploy and uploads the artifact — Pages serves that artifact, not files from
the repo tree. Do **not** set Pages to “Deploy from a branch” (that only serves
`/` or `/docs` from git and will look empty once the static site is replaced
by this pipeline).
